...
struct LITMArrowHeadTests {
    ...
    func testInitialization() {
        ...
        #expect(arrowHead != nil)
        #expect(arrowHead?.arrowColor == color)
    }

    @Test("Initialization with different arrow types", arguments: [0, 1, 2, 4])
    func testInitializationWithDifferentTypes(type: Int) throws {
        let color = NSColor.blue
        let size: Float = 2.0

        let arrowHead = try #require(LITMArrowHead(size: size, color: color, type: Int32(type)))
        #expect(arrowHead.arrowColor == color)
    }

    @Test("Initialization with different sizes", arguments: [0.5, 1.0, 2.0, 10.0])
    func testInitializationWithDifferentSizes(size: Float) throws {
    ...
    func testPositionTransformChanges() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
    ...
    func testMultiplePositioningUpdatesTransform(angle: Float) throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
    ...
    func testPositionAtLineEndpointWithPoints() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
        let points = [
            NSPoint(x: 0.0, y: 0.0),
    ...
    func testStandardArrowType() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
    ...
    func testFlyingArrowType() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 1))

        #expect(arrowHead.path.elementCount > 0)
    }

    @Test("Half arrow left type path creation")
    func testHalfArrowLeftType() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 2))

        #expect(arrowHead.path.elementCount > 0)
    }

    @Test("Half arrow right type path creation")
    func testHalfArrowRightType() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 4))

        #expect(arrowHead.path.elementCount > 0)
    }

    @Test("Scale transform configuration")
    func testScaleTransformConfiguration() throws {
        let smallArrow = try #require(LITMArrowHead(size: 0.5, color: NSColor.red, type: 0))
        let largeArrow = try #require(LITMArrowHead(size: 2.0, color: NSColor.red, type: 0))

        #expect(smallArrow.scaleTransform != largeArrow.scaleTransform)
    }

    @Test("Position transform after positioning")
    func testPositionTransformAfterPositioning() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
        let initialTransform = arrowHead.positionTransform
    ...
    func testMultipleRepositioning() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 0))
    ...
    func testInvalidArrowType() throws {
        let arrowHead = try #require(LITMArrowHead(size: 1.0, color: NSColor.red, type: 999))
