...
struct UserDefaultsKeyTests {
    ...
    func testUniqueRawValues() throws {
        ...
        #expect(rawValues.count == uniqueRawValues.count,
                "Expected \(rawValues.count) unique raw values, but found \(uniqueRawValues.count)")
    }
    ...
    func testRawValueFormat() throws {
        for key in UserDefaultsKey.allCases {
            ...
            #expect(!containsInvalidCharacters,
                    "Key '\(key)' contains invalid characters in raw value '\(rawValue)'")
        }
    }
    ...
    func testNonEmptyRawValues() throws {
        for key in UserDefaultsKey.allCases {
            #expect(!key.rawValue.isEmpty,
                    "Key '\(key)' has an empty raw value")
        }
    }
}
